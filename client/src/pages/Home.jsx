...
import Draggable from 'react-draggable'; // ✅ استيراد مكتبة التحريك
import { API_URL } from "../config";

// --- أيقونات (تم تحديثها لتكون بيضاء ومناسبة لخلفية داكنة) ---
const Icons = {
  ...
  // Story Tools Icons (White for dark bg)
...
const MusicSearchModal = ({ onClose, onSelectMusic }) => {
    ...
    // البحث باستخدام iTunes API المجاني
    const searchMusic = async () => {
        if (!query.trim()) return;
        setSearching(true);
        try {
            // استخدام iTunes Search API لجلب موسيقى حقيقية
    ...
    return (
        <div style={igStyles.musicModal}>
            <div style={igStyles.musicHeader}>
                ...
                <span style={{fontWeight:"bold", fontSize:"16px"}}>Music</span>
                <span style={{flex:1, textAlign:"right", cursor:"pointer", fontSize:"24px"}} onClick={onClose}>✕</span>
            </div>
            ...
            <div style={igStyles.resultsList}>
                {searching ? <div style={{textAlign:"center", padding:"20px"}}>Searching...</div> : results.map(track => (
                    <div key={track.trackId} style={igStyles.trackItem}>
                        <img src={track.artworkUrl60} style={{borderRadius:"5px", marginRight:"10px"}} onClick={() => playPreview(track.previewUrl)}/>
                        <div style={{flex:1, cursor:"pointer"}} onClick={() => { onSelectMusic(track); onClose(); }}>
                            <div style={{fontWeight:"bold", fontSize:"14px"}}>{track.trackName}</div>
...
// --- Story Viewer (عرض الستوري - لا تغيير كبير هنا) ---
const StoryViewer = ({ story, onClose }) => {
    if (!story) return null;
    // استخراج بيانات الموقع إذا كانت محفوظة كـ JSON
    let musicData = null;
    let textData = null;
    try { musicData = story.music ? JSON.parse(story.music) : null; } catch(e) { musicData = { content: story.music, x:0, y:0 }}
    try { textData = story.text ? JSON.parse(story.text) : null; } catch(e) { textData = { content: story.text, x:0, y:0 }}
    ...
    return (
        <div style={{position:"fixed", top:0, left:0, right:0, bottom:0, background:"#000", zIndex:2000, display:"flex", justifyContent:"center", alignItems:"center"}}>
            <div style={{position:"relative", width:"100%", height:"100%", maxHeight:"100vh", maxWidth:"500px", display:"flex", justifyContent:"center", alignItems:"center", overflow:"hidden"}}>
                {story.type === "video" ? (
                    <video src={story.img} autoPlay playsInline style={mediaStyle} />
                ) : (
                    <img src={story.img} style={mediaStyle} alt="story"/>
                )}
                
                {/* عرض العناصر في مواقعها المحفوظة */}
                {textData && textData.content && (
                    <div style={{...igStyles.draggableText, position:"absolute", top:0, left:0, transform: `translate(${textData.x}px, ${textData.y}px)`}}>
                        {textData.content}
                    </div>
                )}
                {musicData && musicData.trackName && (
                    <div style={{...igStyles.draggableMusic, position:"absolute", top:0, left:0, transform: `translate(${musicData.x}px, ${musicData.y}px)`}}>
                        🎵 {musicData.trackName} - {musicData.artistName}
                    </div>
                )}
...
// --- ✅ Story Editor (محرر الستوري الجديد يشبه انستقرام) ---
const StoryEditor = ({ file, fileType, onClose, onUpload }) => {
    const [preview, setPreview] = useState(null);
    const [isUploading, setIsUploading] = useState(false);
    const [showMusicSearch, setShowMusicSearch] = useState(false);
    
    // حالات العناصر القابلة للتحريك (المحتوى + الإحداثيات)
        ...
        <div style={igStyles.editorContainer}>
            ...
            <div style={igStyles.topBar}>
                ...
                <div style={igStyles.topIconsRight}>
                    ...
                    <div onClick={() => { const t = prompt("Enter text:"); if(t) setTextData({...textData, content: t}); }} style={{cursor:"pointer"}}><Icons.TextWhite /></div>
                    <div style={{cursor:"pointer"}}><Icons.StickerWhite /></div>
                </div>
            </div>

            {/* 2. Preview Area (منطقة العرض والسحب) */}
            <div style={igStyles.previewArea}>
                {fileType === "video" ? (
                    <video src={preview} autoPlay loop playsInline muted style={igStyles.previewMedia} />
                ) : (
                    <img src={preview} style={igStyles.previewMedia} />
                )}

                {/* Draggable Text */}
                {textData.content && (
                    <Draggable position={{x: textData.x, y: textData.y}} onStop={handleTextDrag} bounds="parent">
                        <div style={igStyles.draggableText}>{textData.content}</div>
                    </Draggable>
                )}

                 {/* Draggable Music Sticker */}
                {musicData.trackName && (
                    <Draggable position={{x: musicData.x, y: musicData.y}} onStop={handleMusicDrag} bounds="parent">
                        <div style={igStyles.draggableMusic}>
                            <span style={{marginRight:"5px"}}>🎵</span>
                            <div style={{display:"flex", flexDirection:"column"}}>
                                <span style={{fontWeight:"bold"}}>{musicData.trackName}</span>
                                <span style={{fontSize:"10px"}}>{musicData.artistName}</span>
                            </div>
                        </div>
                    </Draggable>
                )}
            ...
            {/* 3. Bottom Bar (زر المشاركة السفلي) */}
            <div style={igStyles.bottomBar}>
                <button 
                    disabled={isUploading}
                    // نرسل البيانات كـ JSON string لحفظ الموقع
                    onClick={() => { setIsUploading(true); onUpload({ text: JSON.stringify(textData), music: JSON.stringify(musicData) }); }} 
                    style={igStyles.shareButton}
                >
                    {isUploading ? "Sharing..." : "Share to Story >"}
                </button>
            </div>

            {/* نافذة البحث عن الموسيقى */}
...
function Home() {
  ...
  const handleUploadStory = async (metadata) => {
      try {
          ...
          // metadata يحتوي الآن على سلاسل JSON للموقع والنص
          ...
          setEditorFile(null); 
      } catch (err) { alert("Failed to upload"); console.error(err); }
  };
...
// --- Styles & Instagram-like Editor Styles ---
const glassStyle = {
...
const igStyles = {
    ...
    previewMedia: { width:"100%", height:"100%", objectFit:"contain" }, // لملء الشاشة مثل انستقرام
    ...
    // Draggable Elements Styles (Instagram Look)
    ...
    draggableMusic: { position:"absolute", color:"black", background:"white", padding:"8px 15px", borderRadius:"12px", fontSize:"14px", cursor:"move", display:"flex", alignItems:"center", boxShadow:"0 2px 10px rgba(0,0,0,0.2)" },

    // Music Modal Styles
    musicModal: { position:"fixed", bottom:0, left:0, right:0, height:"70vh", background:"#222", color:"white", borderTopLeftRadius:"20px", borderTopRightRadius:"20px", zIndex:3100, display:"flex", flexDirection:"column" },
    ...
    searchInput: { background:"transparent", border:"none", outline:"none", color:"white", marginLeft:"10px", flex:1, fontSize:"16px" },
